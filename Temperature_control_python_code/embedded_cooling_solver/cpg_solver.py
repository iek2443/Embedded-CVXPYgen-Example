...
from embedded_cooling_solver import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["lambda_cost", "x_curr", "k_gain", "x_ref"]
    ...
    par.lambda_cost = squeeze_scalar(param_dict["lambda_cost"].value)
    par.x_curr = squeeze_scalar(param_dict["x_curr"].value)
    par.k_gain = squeeze_scalar(param_dict["k_gain"].value)
    par.x_ref = squeeze_scalar(param_dict["x_ref"].value)
    ...
    prob.var_dict['u'].save_value(np.array(res.cpg_prim.u).reshape((1,), order='F'))
    prob.constraints[0].save_dual_value(np.array(res.cpg_dual.d0).reshape((1,), order='F'))
    prob.constraints[1].save_dual_value(np.array(res.cpg_dual.d1).reshape((1,), order='F'))
